var lsCmtTreeClass = new Class({
	...
	typeComment: {
		...
		company: {
			url_add: aRouter.company+'ajaxaddfeedback/',
			url_response: DIR_WEB_ROOT+'/plugins/company/include/ajax/feedbackResponse.php'	
		}
	},
	...
	toggleComment: function(obj,commentId) {
		...
		var thisObj=this;
		JsHttpRequest.query(
        	'POST '+DIR_WEB_ROOT+'/plugins/company/include/ajax/feedbackToggle.php',
	...
	toggleCommentHide: function(obj,commentId) {
		var divContent=$('comment_content_id_'+commentId);
		if (!divContent) {
			return false;
		}
		...
		var thisObj=this;			
		JsHttpRequest.query(
			'POST '+DIR_WEB_ROOT+'/plugins/company/include/ajax/feedbackBad.php',
        	{ idComment: commentId, security_ls_key: LIVESTREET_SECURITY_KEY },
        	function(result, errors) {         		 
            	if (!result) {
                	msgErrorBox.alert('Error','Please try again later');           
        		...
        		if (result.bStateError) {        			
        			...
        			msgNoticeBox.alert(result.sMsgTitle,result.sMsg);     			
        			divContent.removeClass('old').removeClass('self').removeClass('new').removeClass('del');
        			obj.removeClass('hide').removeClass('show');
        			if (result.bState) {
        				divContent.addClass('del');
        				obj.addClass('show');
        			} else {
        				obj.addClass('hide');
        				//document.getElementById('comment_content_id_'+commentId).innerHTML="<font color='#c5c5c5'>отзыв скрыт</font>";
        			}
        			obj.set('text',result.sTextToggle); 
        			var divText = divContent.getChildren('div.text')[0];
        			divText.set('text',result.sMsg);  
