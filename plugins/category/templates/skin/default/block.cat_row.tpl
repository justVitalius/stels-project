<li style="padding-left: {$level}px; {if $sEvent==$oCategory->getUrl()}font-weight: bold;{/if}">
    <a {if $sAdmin}href="#" onclick="lsCategoryTree.Edit('{$oCategory->getId()}','{$oCategory->getParentId()}','{$oCategory->getTitle()}','{$oCategory->getUrl()}');return false;"{else}href="{$oCategory->getCategoryUrl()}"{/if}>
	{$oCategory->getTitle()|escape:'html'}
	<sup>[ {$oCategory->getCountTarget()} ]</sup>

    </a>
{if $sAdmin} <a href="#" onclick="lsCategoryTree.Dell('{$oCategory->getId()}','{$sTypeCategory}');return false; " style="color: red;">&#215;</a>{/if}
</li>
