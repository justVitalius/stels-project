						{assign var="oCompany2" value=$oComment->getTarget()}
						{assign var="oBlog" value=$oCompany2->getBlog()}
						...
						{if !$oComment->getDelete() or $bOneComment or ($oUserCurrent and $oUserCurrent->isAdministrator())}
							...
							<div class="info">
								...
								<ul>
									...
									{if $oUserCurrent and !$oComment->getDelete() and !$bAllowNewComment and ($oBlog->getUserIsJoin() or $oUserCurrent->isAdministrator())}
									...
									{if !$oComment->getDelete() and $oUserCurrent and $oUserCurrent->isAdministrator()}   										
   									...
   									{/if}
   									{if !$oComment->isBad() and $oUserCurrent and ($oBlog->getUserIsAdministrator() or $oBlog->getUserIsModerator() or $oUserCurrent->isAdministrator())}   										
										<li><a href="#" class="hide" onclick="lsCmtTree.toggleCommentHide(this,{$oComment->getId()}); return false;">{$aLang.comment_collapse}</a></li>
									{/if}
									{if $oComment->isBad() and $oUserCurrent and ($oBlog->getUserIsAdministrator() or $oBlog->getUserIsModerator() or $oUserCurrent->isAdministrator())}   										
										<li><a href="#" class="show" onclick="lsCmtTree.toggleCommentHide(this,{$oComment->getId()}); return false;">{$aLang.comment_expand}</a></li>
									{/if}	
						...
						{else}				
							<span class="delete">{$aLang.company_feedbacks_was_delete}</span><br><br>
						{/if}		
